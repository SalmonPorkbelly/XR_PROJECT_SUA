...
public class HUDTextManager : MonoBehaviour
{
    ...
    public Vector3 offset; //캐릭터 머리위에 텍스트를 표시할 오프셋

    public GameObject HudTextUp; // 올라가는 텍스트 프리맵
    public GameObject canvasObject; // canvas transform 좌표
    ...
    void Update()
    {
        if (character != null){
            ...
            Vector3 screenPosition = Camera.main.WorldToScreenPoint(characterHeadPosition); //3D position -> 2D cam
    ...
    public void UpdateHUDTextSet(string newText, GameObject target, Vector3 TargetOffset)
    {
        ...
        Vector3 screenPosition = Camera.main.WorldToScreenPoint(TargetPosition); //3D position -> 2D cam
